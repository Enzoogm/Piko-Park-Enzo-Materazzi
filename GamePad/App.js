...
export default function App() {
  ...
  const [direccionIp, setDireccionIp] = useState("192.168."); // Ya te dejé tu IP por defecto!
  ...
  const conectarAlServidor = () => {
    ...
    // Ahora el usuario escribe IP + PUERTO (ej: 192.168.1.39:3000)
    const urlSocket = `http://${direccionIp}`;

    // IMPORTANTE: sin forzar websocket
    ...
    socketRef.current.on("connect_error", (error) => {
      ...
      setEstaConectado(false);
      setEstadoDeConexion("No se pudo conectar");
    });
  };
